...
### ميزانية المعالج (CPU) — اقرأها قبل تغيير الـ SHA

Supabase بتدّي كل طلب حوالي **2 ثانية CPU**. المحرك في الـ commits القديمة
كان بيحتاج 3.6–5.7 ثانية لرد «اهلا» على بارد، فكل رسالة SIE على الموقع كانت
بتموت بـ **546 «CPU Time exceeded»** — وده بعد ما `sie_consume_message`
تكون خصمت الرسالة من حصة العميل. من `cba70f3` نفس الدور بياخد ~190ms على
بارد و~10ms بعد كده.

قبل أي ترقية، قِس الدور البارد محليًا (Node، مع `fetch` لملفات `file:`
ومحاكي Supabase) وتأكد إنه بعيد عن حد الثانيتين.
